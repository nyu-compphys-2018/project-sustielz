...
######## Initialization ########
    def __init__(self, N, M):              ##Initialize an NxM lattice, consisting of fluid with walls at edges.
        ...
        RIGHT = [];                 ##Initialize four walls. (Note: walls exclude corners)
        for j in range(M):
            RIGHT.append([N, j]);
        self.boundaries.append(RIGHT);
        UP = [];
        for i in range(N):
            UP.append([i, M]);
        self.boundaries.append(UP);
        LEFT = [];                 ##Initialize four walls. (Note: walls exclude corners)
        for j in range(M):
            LEFT.append([-1, j]);
        self.boundaries.append(LEFT);
        DOWN = [];  
        for i in range(N):
            DOWN.append([i, -1]);
        self.boundaries.append(DOWN);
##        print UP
##        print DOWN
##        print LEFT
##        print RIGHT
        self.corners = [[N, M], [-1, M], [-1, -1], [N, -1]] ##Initialize list of corners
...
##                    print "i, j =", i, j;
    ...
    def stream2(self, I, xvals, yvals): ##Stream all points (i, j) in (xvals, yvals) in the direction I
        ex, ey = self.dct[I];
        for i in xvals:
            for j in yvals:
                self.f[I][i+ex, j+ey] = self.fs[I][i, j];
    ...
    def stream_Walls(self):
        ...
        for I in [3, 6, 7, 2, 4]:  ##Stream right wall
            self.stream(I, self.boundaries[0]);
        for I in [4, 7, 8, 1, 3]:  ##Stream top wall
            self.stream(I, self.boundaries[1]);
        for I in [1, 5, 8, 2, 4]:  ##Stream left wall
            self.stream(I, self.boundaries[2]);
        for I in [2, 5, 6, 1, 3]:  ##Stream bottom wall
            self.stream(I, self.boundaries[3]);

  

        for I in [7, 4, 3]:
            self.stream(I, [self.corners[0]]) ###Stream top right corner
        for I in [8, 4, 1]:
            self.stream(I, [self.corners[1]]) ###Stream top right corner
        for I in [5, 2, 1]:
            self.stream(I, [self.corners[2]]) ###Stream top right corner
        for I in [6, 2, 3]:
            self.stream(I, [self.corners[3]]) ###Stream top right corner


    def stream_Walls2(self):
        N, M = self.N, self.M;
